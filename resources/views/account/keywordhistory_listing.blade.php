//resources/views/account/keywordhistory_listing.blade.php
@if ($keyword_histories->count() > 0)
    <table class="table table-box pml-table mt-2"
        current-page="{{ empty(request()->page) ? 1 : empty(request()->page) }}"
    >
        @foreach ($keyword_histories as $key => $item)
            <tr class="position-relative">
                ...
                <td class="text-end">
                    <div class="pull-right">
                        ...
                        <span class="text-muted2">{{ Auth::user()->customer->formatDateTime($item->created_at, 'datetime_full') }}</span>
                    </div>
                </td>
            </tr>
        @endforeach
    </table>
    @include('elements/_per_page_select', ["items" => $keyword_histories])
